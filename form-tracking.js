(function () {
      ...
      if (submitButton && !submitButton.hasAttribute("data-gtm-tracked")) {
        ...
        submitButton.addEventListener("click", function (event) {
          ...
          const formData = {
            ...
            has_brokerage: !!document.querySelector('[data-input="brokerage"]')
              ?.value,
          };
  ...
  function initTracking() {
    ...
    if (currentPath === "/form-agent" || currentPath.includes("/form-agent")) {
      ...
      console.log("🔍 Setting up form-v2 tracking");
      ...
      currentPath === "/submit-agent-success" ||
      currentPath.includes("/submit-agent-success")
    ) {
  ...
  function setupFormSubmitListener() {
    ...
    if (homeDataValidationButton) {
      homeDataValidationButton.addEventListener("click", function (event) {
        ...
        const isValid =
          typeof checkInputsValidity === "function"
            ? checkInputsValidity()
            : true;
        ...
        if (isValid) {
          const formData = {
            home_type:
              document.querySelector('[data-input="type-of-home"]')?.value ||
              "",
            bedrooms:
              document.querySelector('[data-input="bedrooms"]')?.value || "",
            bathrooms:
              document.querySelector('[data-input="bathrooms"]')?.value || "",
            mortgage_type:
              document.querySelector('[data-input="mortgage-type"]')?.value ||
              "",
            square_footage:
              document.querySelector('[data-input="square-foot"]')?.value || "",
          };
    ...
    const step3SubmitButton = document.querySelector(
    ...
    if (step3SubmitButton) {
      step3SubmitButton.addEventListener("click", function (event) {
        ...
        const isValid =
          typeof checkInputsValidity3 === "function"
            ? checkInputsValidity3()
            : true;

        if (isValid) {
          // Get the selected values from step 3 form
          const homeConditionDiv = document.querySelector(
            '[data-input="home-condition"]'
          );
          const homeCondition =
            homeConditionDiv?.querySelector("[data-select].is-active div")
              ?.textContent || "";

          const solarDiv = document.querySelector('[data-input="solar"]');
          const solar =
            solarDiv?.querySelector("[data-select].is-active div")
              ?.textContent || "";
          ...
          const formData = {
            ...
            solar: solar,
            garage:
              document.querySelector('[data-input="garage"]')?.value || "",
            pool: document.querySelector('[data-input="pool"]')?.value || "",
            time_to_move:
              document.querySelector('[data-input="months-to-move"]')?.value ||
              "",
          };
    ...
    const step4SubmitButton = document.querySelector(
    ...
    if (step4SubmitButton) {
      step4SubmitButton.addEventListener("click", function (event) {
        ...
        if (isValid) {
          ...
          const includesTaxesIns =
            taxesInsDiv?.querySelector("[data-select].w--current div")
              ?.textContent || "";
          ...
          const formData = {
            ...
            monthly_mortgage_payment:
              document.querySelector('[data-input="monthly-mortgage-payment"]')
                ?.value || "",
            includes_taxes_insurance: includesTaxesIns,
            annual_property_taxes:
              document.querySelector('[data-input="annual-property-taxes"]')
                ?.value || "",
            homeowners_insurance:
              document.querySelector('[data-input="homeowners-insurance"]')
                ?.value || "",
            monthly_hoa:
      ...
      finalSubmitButton.addEventListener("click", function (event) {
        ...
        if (currentStep === "5") {
          ...
          if (isValid && legalChecked) {
            const formData = {
              discovery_source:
                document.querySelector('[data-input="discovery-source"]')
                  ?.value || "",
              firstName:
                document.querySelector('[data-input="first-name"]')?.value ||
                "",
              lastName:
                document.querySelector('[data-input="last-name"]')?.value || "",
              has_email: !!document.querySelector('[data-input="email"]')
                ?.value,
              has_phone: !!document.querySelector('[data-input="phone"]')
                ?.value,
              passes_step4_validation:
                !!sessionStorage.getItem("step4DataPass"),
            };
